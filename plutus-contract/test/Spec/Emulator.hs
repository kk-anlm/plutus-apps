...
import Ledger (CardanoTx (..), OnChainTx (Valid), PaymentPubKeyHash, ScriptContext, ScriptError (EvaluationError),
               TxOut (txOutValue), ValidationError (ScriptFailure), Validator, Value, mkValidatorScript, scriptTxIn,
               scriptTxOut, txOutRefs, unitDatum, unitRedeemer, unspentOutputs)
import Ledger.Generators (Mockchain (Mockchain))
import Ledger.Generators qualified as Gen
import Ledger.Index qualified as Index
import Ledger.Typed.Scripts (wrapValidator)
import Ledger.Value qualified as Value
import Legacy.Plutus.V2.Ledger.Tx (Tx (txFee, txMint, txOutputs), outputs)
...
invalidTrace = property $ do
    (Mockchain m _ _, txn) <- forAll genChainTxn
    let invalidTxn = txn { txMint = Value.adaValueOf 1 }
...
pubKeyTransactions2 = do
    let payment1 = initialBalance P.- Value.adaValueOf 10
        payment2 = initialBalance P.+ Value.adaValueOf 10
    ...
    _ <- Trace.nextSlot
    Trace.liftWallet wallet1 $ payToPaymentPublicKeyHash_ W.always (Value.adaValueOf 20) pubKey2
    void Trace.nextSlot
...
initialBalance = Value.adaValueOf 100
